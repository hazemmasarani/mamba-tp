...
class MambaMixer(nn.Module):
    ...
    def __init__(self, config: MambaConfig, layer_idx: int, initialize_mixer_weights: bool = True):
        ...
        self.in_proj = nn.Linear(self.hidden_size, self.local_intermediate_size * 2, bias=config.use_bias)
        ...
        self.A_log = nn.Parameter(torch.empty(self.local_intermediate_size, self.ssm_state_size))
        self.D = nn.Parameter(torch.empty(self.local_intermediate_size))
    ...
    def init_mamba_weights(self):
        ...
        dt = torch.exp(
            torch.rand(self.local_intermediate_size, device=self.dt_proj.bias.device, dtype=torch.float32)
...
class MambaModel(MambaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        # torch.save(input_ids, "log/orig/input_ids.pt")
